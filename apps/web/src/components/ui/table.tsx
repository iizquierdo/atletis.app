...
function TableHeader({ className, ...props }: React.HTMLAttributes<HTMLTableSectionElement>) {
  return (
    <thead data-slot="table-header" className={cn('[&_tr]:border-b [&_tr]:border-foreground/10', className)} {...props} />
  );
}
...
function TableHead({ className, ...props }: React.ThHTMLAttributes<HTMLTableCellElement>) {
  return (
    <th
      ...
      className={cn(
        'h-12 bg-table-header px-4 text-left rtl:text-right align-middle font-medium uppercase tracking-wide text-table-header-foreground [&:has([role=checkbox])]:pe-0',
        className,
      )}
      {...props}
    />
