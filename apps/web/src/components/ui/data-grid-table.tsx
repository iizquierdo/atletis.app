...
function DataGridTableHeadRow<TData>({
  ...
  return (
    <tr
      key={headerGroup.id}
      className={cn(
        props.tableLayout?.headerBackground === false ? 'bg-transparent' : 'bg-table-header',
        props.tableLayout?.headerBorder && '[&>th]:border-b [&>th]:border-foreground/10',
...
function DataGridTableHeadRowCell<TData>({
  ...
  return (
    <th
      ...
      className={cn(
        'relative h-10 text-left rtl:text-right align-middle font-medium uppercase tracking-wide text-table-header-foreground [&:has([role=checkbox])]:pe-0',
        headerCellSpacing,
        props.tableLayout?.cellBorder && 'border-e border-foreground/10',
        props.tableLayout?.columnsResizable && column.getCanResize() && 'truncate',
        props.tableLayout?.columnsPinnable &&
          column.getCanPin() &&
          '[&:not([data-pinned]):has(+[data-pinned])_div.cursor-col-resize:last-child]:opacity-0 [&[data-last-col=left]_div.cursor-col-resize:last-child]:opacity-0 [&[data-pinned=left][data-last-col=left]]:border-e! [&[data-pinned=right]:last-child_div.cursor-col-resize:last-child]:opacity-0 [&[data-pinned=right][data-last-col=right]]:border-s! [&[data-pinned][data-last-col]]:border-foreground/15 data-pinned:bg-table-header data-pinned:backdrop-blur-xs',
  ...
  return (
    <div
      {...{
        ...
        className:
          'absolute top-0 h-full w-4 cursor-col-resize user-select-none touch-none -end-2 z-10 flex justify-center before:absolute before:w-px before:inset-y-0 before:bg-foreground/25 before:-translate-x-px',
      }}
    />
  );
}
